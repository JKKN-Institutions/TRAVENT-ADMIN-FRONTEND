...
const ReviewForm = ({ data, onSubmit }) => {
  ...
  return (
    <div className="form-container">
            ...
            <div key={j}>
              ...
              {department.years.map((year, k) => (
                <div key={k}>
                  ...
                  <p><strong>Sections:</strong> {year.sections.split(',').join(', ')}</p>
                </div>
              ))}
            </div>
          ))}
      ...
      <h3>Admin Details:</h3>
      <p><strong>Name:</strong> {adminDetails.adminName}</p>
      <p><strong>Email:</strong> {adminDetails.email}</p>
      <p><strong>Contact Number:</strong> {adminDetails.contactNumber}</p>

      <button onClick={onSubmit} className="form-button">Submit Data</button>
    </div>
  );
};
